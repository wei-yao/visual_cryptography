...
public class MainFrame {
	...
	private void initializeImagePos(){
		...
		imagePos[4][1]=imagePos[3][1]=imagePos[2][1]=imagePos[1][1]=imagePos[0][1]+MySketch.DEFAULT_SIZE+MySketch.PADDING*2+MySketch.BUTTON_HEIGHT;
		imagePos[2][0]=imagePos[1][0]+MySketch.DEFAULT_SIZE+MySketch.PADDING;
		imagePos[3][0]=imagePos[2][0]+MySketch.DEFAULT_SIZE+MySketch.PADDING;
		imagePos[4][0]=imagePos[3][0]+MySketch.DEFAULT_SIZE+MySketch.PADDING;
	}
	/**
	 * 初始化button位置矩形的坐标数组.
	 */
...
//		buttonPos[4][1] = buttonPos[3][1] = buttonPos[2][1] = buttonPos[1][1];
...
//				+ MySketch.DEFAULT_SIZE;
		for(int i=0;i<buttonPos.length;i++){
			buttonPos[i][0]=imagePos[i][0]+MySketch.DEFAULT_SIZE/2-MySketch.BUTTON_WIDTH/2;
			buttonPos[i][1]=imagePos[i][1]+MySketch.DEFAULT_SIZE+MySketch.PADDING;
		}
	}
	...
	public void draw() {
		for(int i=0;i<5;i++){
				pApplet.image(imgs[i],imagePos[i][0],imagePos[i][1],MySketch.DEFAULT_SIZE,MySketch.DEFAULT_SIZE);
		}
			pApplet.image(overlayResult, RESULT_X,RESULT_Y,MySketch.OUTPUT_SIZE,MySketch.OUTPUT_SIZE);
		drawButtons();
	...
	private void drawButtons() {
		...
		pApplet.fill(255);
		for(int i=0;i<5;i++){
			pApplet.rect(buttonPos[i][0],buttonPos[i][1] , MySketch.BUTTON_WIDTH, MySketch.BUTTON_HEIGHT);
		}
	}
	...
	private void handleCarrierImageClick(int id) {
		if(isLoaded[id]&&!isOverlayed[id])
		{
			overlayImage(id);
		}
	}
	private void overlayImage(int id) {
	   pApplet.blend(imgs[id], imagePos[id][0],imagePos[id][1], imgs[id].width,imgs[id].height, RESULT_X, RESULT_Y, MySketch.OUTPUT_SIZE, MySketch.OUTPUT_SIZE, PApplet.BURN);
	}
	/**
	 * 清除叠加的结果.
	 */
...
//		if(image!=null)
	...
	private PImage overlayResult=new PImage(MySketch.OUTPUT_SIZE, MySketch.OUTPUT_SIZE);
	private static final int RESULT_X=MySketch.PADDING*4+MySketch.DEFAULT_SIZE*3;
